...
import {veryLightGray} from 'src/styleguide.js'
      ...
      <Scoped css={css}>
        <div>
          {(!cases || cases.length === 0) ? (
            <div className="row">
              <div>
                No cases found. <Link to="/app/cases/create">Create a new case</Link>.
              </div>
            </div>
          ) : cases.map(kase =>
            <div className="row" key={kase.id}>
              ...
              <div>
                {kase.id === activeCase.id ? (
                  ...
                  <button className="primary small" onClick={partial(chooseCase, kase)}>
                    Work on case
                  </button>
                )}
