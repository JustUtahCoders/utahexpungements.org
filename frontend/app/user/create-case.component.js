...
import { success } from "frontend/styleguide.js";
...
export default class CreateCase extends React.Component {
  ...
  onSubmit = event => {
    ...
    if (newPerson) {
      ...
      database
        ...
        .then(personRef => {
          personRef.get().then(personSnapshot => {
            ...
            this.createCase(uid, personRef.id, caseNickname).then(
              partial(this.doneCreatingCase, person)
            );
          });
        })
      ...
      this.createCase(uid, selectedPerson.id, caseNickname).then(
        partial(this.doneCreatingCase, selectedPerson)
      );
    }
  };
  ...
  createCase = (uid, personId, caseNickname) => {
    return database
      .createCase(uid, personId, caseNickname, true)
      .catch(this.catchError);
  };
      ...
      <Scoped css={css}>
        <div>
          ...
          <form className="create-case-form" onSubmit={this.onSubmit}>
            <fieldset>
              <legend>
                Is this case for an existing person or a new person?
              </legend>
              <div>
                <input
                  type="radio"
            ...
            <div>
              <p>
                Choose a nickname for this case. You will be able to enter more
                data about the case later.
              </p>
              <input
                type="text"
                style={{ width: "100%" }}
          ...
          {success && (
            <div className="success-message">
              <p>{success} You are now working on this case.</p>
              <p>
                <Link to="/app/dashboard">View all your cases</Link> or{" "}
                <Link to="/app/tool/certificate-of-eligibility">
                  get started
                </Link>{" "}
                on this case.
              </p>
